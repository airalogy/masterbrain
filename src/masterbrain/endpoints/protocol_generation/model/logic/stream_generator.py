...
async def generate_stream(protocol_msg: ModelProtocolMessage, history: list):
    start_time = time.time()

    # Prepare user prompt
    prompt = USER_MESSAGE_MODEL_PY_HEAD_TEMPLATE + PromptTemplate(
        input_variables=["USER_MESSAGE_PROTOCOL_AIMD"],
        template=USER_MESSAGE_MODEL_PY_TAIL_TEMPLATE,
    ).format(USER_MESSAGE_PROTOCOL_AIMD=protocol_msg.protocol_aimd)
    ...
    end_time = time.time()
    print(f"Model.py Generation Total Time: {end_time - start_time:.6f} s")
