...
- 新增可复用的异步 `masterbrain.embeddings` API 和 `POST /api/endpoints/embeddings`，通过服务端配置的 Qwen 或 OpenAI 供应商完成有界文本批量向量化。
- 向量化接入现有宿主用量事件，涵盖请求关联、上游失败与取消，用量事件不保存原文或向量。
- 新增请求与向量校验、明确的维度保留以及离线 HTTP 级回归，验证每次向量化仅尝试一次上游请求，不隐式切换模型。

### 集成说明

- 明确 Masterbrain 可复用智能能力与宿主权限、预算、资产、索引及执行之间的边界；Platform 现有 `text-embedding-v4` 索引可继续使用 1024 维向量空间。

## v0.11.0

### 新增功能

- 新增版本化的代码修改响应契约，包含稳定变更集 ID、逐文件 SHA-256、明确的问答/修改结果和风险建议，便于宿主应用统一接入。
- 新增带乐观冲突检查和回滚能力的原子工作区修改，使宿主能够完整应用或撤销 AI 变更集，不会遗留部分文件修改。

### 变更

- 代码修改响应现在会区分普通问答和文件变更，并将操作分为安全、警告和破坏性风险，使宿主能够自动应用安全修改并审核高风险修改。
- Masterbrain Studio 改为使用共享客户端与 Vue 能力包，实现自动应用、变更审核、全尺寸 Monaco Diff 和冲突安全撤销。

## v0.10.1

### 问题修复

- OpenCode 对话修改运行时的本地健康检查与消息请求不再受进程级 HTTP 代理影响，并增加可配置的启动超时和可操作的启动日志尾部信息。
- Protocol 生成内容保持用户要求的语言，并禁止生成或对话修改产生 assigned fields 为空的无效“仅校验” assigner。
...
- 新增 `GET /api/health/providers`，可在不生成模型内容的情况下检查 provider 凭据、连通性和默认模型权限。

### 问题修复

- 修复 Protocol 生成 prompt 中 AIMD 模板双花括号在模型调用前被错误折叠的问题。

## v0.9.0

### 新增功能

- 新增 OpenCode-backed `endpoints/code_edit`，支持在 Airalogy Protocol Editor 中基于当前 `protocol.aimd`、`model.py`、`assigner.py` 与 `protocol.toml` 进行对话式代码理解和修改。
- `code_edit` 现在支持 `workspace_id`，可按编辑会话复用 OpenCode server 进程，同时每次请求仍创建新的 OpenCode session，并以浏览器传入的完整文件快照作为 source of truth。

### 安全与稳定性

- 将 `code_edit` 的文件变更范围限制到当前 Protocol Editor 的四个核心文件，忽略 helper 文件等越界修改。
- 对返回的 AIMD、Python 和 TOML 修改结果执行语法校验，并把风险作为 warning 返回给调用方。
- 新增会话级 OpenCode runtime manager：支持 idle timeout 清理、全局 runtime 上限、用户 namespace 上限、同一 workspace 串行锁和服务关闭时统一清理。
- 收紧 `code_edit` execution log，避免向前端暴露服务器临时目录、OpenCode binary 路径和 localhost 端口。
