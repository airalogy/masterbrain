...
- `Source Run` means you can run `uv run masterbrain-studio` from a source checkout.
...
- Source builds depend on published `@airalogy/aimd-*` npm packages when building or running the Studio frontend. A sibling `aimd/` repository is not required.
