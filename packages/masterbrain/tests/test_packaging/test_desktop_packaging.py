...
from masterbrain.desktop import _desktop_argv, _warn_if_deprecated_entrypoint
...
def test_deprecated_desktop_entrypoint_warns(monkeypatch, capsys) -> None:
    monkeypatch.setattr("sys.argv", ["/tmp/masterbrain-desktop"])

    _warn_if_deprecated_entrypoint()

    captured = capsys.readouterr()
    assert "masterbrain-desktop" in captured.err
    assert "masterbrain-studio" in captured.err


def test_studio_entrypoint_does_not_warn(monkeypatch, capsys) -> None:
    monkeypatch.setattr("sys.argv", ["/tmp/masterbrain-studio"])
