__all__ = ["generate_phased_conclusion"]
...
async def generate_phased_conclusion(
    workflow_data: WorkflowData, model: str = DEFAULT_MODEL, debug: bool = DEBUG
) -> ConclusionData:
    assert (
        workflow_data.path_data.path_status == "waiting_for_phased_research_conclusion"
    ), "Workflow path status must be 'waiting_for_phased_research_conclusion'."
    ...
    if debug:
        print_with_border(f"Generated phased conclusion data:\n{content}")
